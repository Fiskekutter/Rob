...
class crustcrawlerNode:
	...
	def moveToPoint(self):
		self.setInverseKinematics(calibrated_Table)
	...
	def setCalibration(self):
		self.calibrated_Table = [xyz_Table[0]-offset_Table[0],
					xyz_Table[1]-offset_Table[1],
					xyz_Table[2]-offset_Table[2]]
	...
	def sendCommand(self):
		...
		self.client.send_goal(self.gripperGoal)		
	...
	def moveRobot(self, data):
		self.setXYZTable([data[0],data[1]])
		self.setCalibration()
		self.moveToPoint()
		self.closeGripper()
		self.setXYZTable([Data[2],data[3]])
		self.setCalibration()
		self.moveToPoint()
		self.openGripper()
		self.homePos()
	...
	def calibrateRobot(self, data):
		self.setOffsetTable(data)

	def main(self, argv):
		rospy.init_node(self.node)
		self.subToMove = ros.subscriber(self.topic + "/rob/move", std_msgs.msg.FloatMultiArray, self.moveRobot)
		self.subToHome = ros.subscriber(self.topic + "/rob/home", std_msgs.msg.Empty, self.homePos)
		self.subToZero = ros.subscriber(self.topic + "/rob/zero", std_msgs.msg.FloatMultiArray, self.calibrateRobot)
		rospy.spin()
